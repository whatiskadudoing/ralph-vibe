...
import { Box, Text, render, useInput, useApp } from "../../packages/deno-ink/src/mod.ts";
import {
  CommandBox,
  Header,
  KeyboardHints,
...
function Checkbox({ label, description, checked, focused, disabled }: CheckboxProps): React.ReactElement {
...
export function printAudienceInterviewHeader(): void {
  console.log();
  console.log(`${ansi.orange}◆${ansi.reset} ${ansi.bold}Audience & JTBD Interview${ansi.reset} ${ansi.dimGray}[1/2]${ansi.reset}`);
  console.log();
  console.log(`${ansi.dimGray}First, let's understand WHO you're building for and WHAT they need${ansi.reset}`);
  console.log();
}

export function printAudienceCompleted(): void {
  console.log();
  console.log(`${ansi.orange}◆${ansi.reset} ${ansi.bold}Audience Documented!${ansi.reset} ${ansi.green}✓${ansi.reset}`);
  console.log();
  console.log(`${ansi.dimGray}→${ansi.reset} ${ansi.orange}AUDIENCE_JTBD.md${ansi.reset} ${ansi.dimGray}created${ansi.reset}`);
  console.log();
}

export function printSpecsInterviewHeader(isStep2: boolean): void {
  console.log();
  console.log(`${ansi.orange}◆${ansi.reset} ${ansi.bold}Activity Spec Interview${ansi.reset}${isStep2 ? ` ${ansi.dimGray}[2/2]${ansi.reset}` : ""}`);
  console.log();
  console.log(`${ansi.dimGray}Now let's define the activities (features) users will do${ansi.reset}`);
  console.log();
}
...
export function printCompletionSummary(usageInfo?: string, vibeMode?: boolean): void {
  ...
  console.log();
  console.log(`${ansi.dimGray}→${ansi.reset} Your specifications are in ${ansi.orange}specs/${ansi.reset}`);
  ...
  if (!vibeMode) {
    ...
    console.log(`  ${ansi.orange}ralph plan${ansi.reset}${ansi.gray} to generate implementation plan${ansi.reset}`);
  }
  console.log();
}
...
export async function renderStartOptions(hasAudienceAlready: boolean): Promise<RenderStartOptionsResult> {
