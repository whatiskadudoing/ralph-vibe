...
export function CostBadge({
  ...
  const deltaStr = iterationCost && iterationCost > 0
  ...
  if (compact) {
    return (
      <Box flexDirection="row">
        ...
        {deltaStr && (
          <Text color={colors.dim}> ({deltaStr})</Text>
        )}
        {trendIndicator && (
          <Text color={colors.warning}>{trendIndicator}</Text>
        )}
      </Box>
    );
  }
...
export function SessionCost({
  ...
  const trendIcon = trend === "very-high"
    ? "⚠"
    : trend === "high"
    ? "↑"
    : "";
