...
import { createNode, squashTextNodes, isTextNode } from "./src/dom.ts";
...
function rebuildYogaTree(node, parentYogaNode, y) {
  ...
  if (node.nodeName === "ink-text" && !hasYogaChildren) {
    yogaNode.setMeasureFunc((width, _widthMode, _height, _heightMode) => {
      ...
      console.log(`  Measure: text="${text}", width=${width} -> ${textWidth}x${textHeight}`);
...
function printDom(node, indent = "") {
  if (isTextNode(node)) {
    ...
    const dims = yn ? `w=${yn.getComputedWidth()}, h=${yn.getComputedHeight()} at (${yn.getComputedLeft()},${yn.getComputedTop()})` : "no yoga";
    const styleInfo = node.style?.wrap ? ` wrap=${node.style.wrap}` : "";
    console.log(indent + node.nodeName + " [" + dims + "]" + styleInfo);
    for (const child of node.childNodes) {
